...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self._img_label = tk.Label(left, bg="#0f3460",
                                   width=150, height=150)
        self._img_label.pack()

        btn_col = tk.Frame(left, bg="#1a1a2e")
        btn_col.pack(pady=(8, 0))

        self._open_btn = tk.Button(
            btn_col, text="Open Image",
            command=self._open_image,
            font=("Helvetica", 9, "bold"),
            bg="#e94560", fg="white",
            activebackground="#c0392b", activeforeground="white",
            relief="flat", padx=10, pady=5, cursor="hand2", width=12
        )
        self._open_btn.pack(pady=(0, 4))

        self._pred_btn = tk.Button(
            btn_col, text="Classify",
        ...
        # right column
        right = tk.Frame(body, bg="#16213e", padx=10, pady=10)
        ...
        self._result_label = tk.Label(
            ...
            fg="#e0e0e0", bg="#16213e", wraplength=260, justify="left"
        )
        self._result_label.pack(anchor="w", pady=(2, 8))
        ...
        for _ in range(5):
            ...
            lbl = tk.Label(row, text="", font=("Helvetica", 7),
                           fg="#bdc3c7", bg="#16213e", width=22, anchor="w")
            lbl.pack(side="left")

            bg = tk.Frame(row, bg="#2c3e50", height=10, width=120)
            bg.pack(side="left", padx=3)
            bg.pack_propagate(False)

            bar = tk.Frame(bg, bg="#2ecc71", height=10, width=0)
    ...
    def _open_image(self):
        ...
        self._img_label.configure(image=self._img_tk, width=150, height=150)
    ...
    def _update_ui(self, results):
        ...
        for i, (lbl, _, bar, pct) in enumerate(self._bar_widgets):
            if i >= len(results):
                ...
                continue
            n, c = results[i]
            lbl.configure(text=n[:22])
            bar.configure(bg=_bar_color(c))
            bar.place(width=max(int(c * 120), 1))
